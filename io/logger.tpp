...
std::string Logger<OutStream>::getTime() const {
...
template<typename OutStream>
Logger<OutStream>::Logger(OutStream &outputStream) : outputStream(outputStream) {}
...
template<typename OutStream>
void Logger<OutStream>::startTime() {
    startTimeMs = now<std::chrono::nanoseconds>();
}

template<typename OutStream>
void Logger<OutStream>::endTime() {
    endTimeMs = now<std::chrono::nanoseconds>();
}

template<typename OutStream>
void Logger<OutStream>::printElapsedTime() {
...
void Logger<OutStream>::measure(Callable &&callable, unsigned int iterations, std::string_view name) {
    ...
    ("Average: ", totalTime.count() / static_cast<float>(iterations), " ", durationToString<Resolution>());
}
