...
Range<T>::Range(T start, U end, V step) : _start(start), _end(end), _step(step) {}
...
template<typename T>
Range<T>::iterator::iterator(T value, T step, T max) : value(value), step(step), max(max) {}
...
template<typename T>
typename Range<T>::iterator &Range<T>::iterator::operator=(const Range::iterator &other) {
    value = other.value;
}
